...
class ImageLabelingApp:
    def __init__(self, root):
        ...
        # 이미지 확대/축소 및 이동을 위한 변수
        self.image_scale = 1.0  # 이미지의 현재 스케일 (배율)
        self.image_offset_x = 0  # 이미지의 X 축 오프셋 (이동 위치)
        self.image_offset_y = 0  # 이미지의 Y 축 오프셋 (이동 위치)
        self.start_x = 0  # 드래그 시작 시의 X 좌표
        self.start_y = 0  # 드래그 시작 시의 Y 좌표
        
        self.canvas.bind("<MouseWheel>", self.zoom_image)  # 확대/축소
        self.canvas.bind("<ButtonPress-1>", self.start_drag)  # 드래그 시작
        self.canvas.bind("<B1-Motion>", self.drag_image)  # 드래그 중
            
        # Edge 표시 상태를 저장하는 딕셔너리
        self.edge_visibility = {}

        # 전체 탭을 대상으로 하는 버튼 프레임 생성
        global_button_frame = tk.Frame(self.notebook_frame)
        global_button_frame.pack(fill=tk.X, pady=5)

        # 현재 선택된 탭을 대상으로 하는 버튼 프레임 생성
        local_button_frame = tk.Frame(self.notebook_frame)
        local_button_frame.pack(fill=tk.X, pady=5)

        # "현재 탭 모두 선택" 버튼 추가
        self.local_select_all_button = tk.Button(local_button_frame, text="Select All (Ctrl + A)", command=self.select_all_edges)
        self.local_select_all_button.pack(side=tk.LEFT, padx=5)
        # Ctrl + A 키 조합을 desellect_all 함수에 바인딩
        self.root.bind("<Control-a>", lambda event: self.select_all_edges())

        # "현재 탭 모두 해제" 버튼 추가
        self.local_deselect_all_button = tk.Button(local_button_frame, text="Deselect All (Ctrl + D)", command=self.deselect_all_edges)
        self.local_deselect_all_button.pack(side=tk.LEFT, padx=5)
        # Ctrl + D 키 조합을 desellect_all 함수에 바인딩
        self.root.bind("<Control-d>", lambda event: self.deselect_all_edges())
        
    def zoom_image(self, event):
        # Windows에서 휠 업은 +120, 휠 다운은 -120
        if event.delta > 0:
            self.image_scale *= 1.1  # 확대
        else:
            self.image_scale /= 1.1  # 축소
        self.display_image()
    ...
    def drag_image(self, event):
        # 드래그 이벤트에 따라 이미지 위치 변경
        self.image_offset_x += event.x - self.start_x
        self.image_offset_y += event.y - self.start_y
        self.start_x = event.x
        self.start_y = event.y
        self.display_image()
    ...
    def show_previous_image(self):
        ...
        if current_index > 0:
            ...
            self.relation_triple_info_initialized = False
            self.last_selected_tab = None
            self.reset_zoom()  # 이미지 선택 시 확대/축소 초기화
            self.display_image()
    ...
    def display_image(self):
        ...
        # 중앙 패널의 너비에 맞춰 이미지 리사이즈 및 확대/축소 적용
        canvas_width = self.canvas.winfo_width()
        canvas_height = self.canvas.winfo_height()
        aspect_ratio = image.height / image.width
        new_width = int(canvas_width * self.image_scale)
        ...
        # 이미지를 중앙으로 이동 및 오프셋 적용
        x_offset = (canvas_width - new_width) // 2 + self.image_offset_x
        y_offset = (canvas_height - new_height) // 2 + self.image_offset_y
    ...
    def display_relation_triples(self):
        ...
        # "모두 선택" 및 "모두 해제" 버튼을 포함하는 프레임 생성
        button_frame = tk.Frame(self.notebook_frame)
        ...
        for predicate in self.predicates:
            ...
            if triples:
                ...
                for triple in triples:
                    ...
                    # 기존 상태 유지
                    if triple_key in self.edge_visibility:
                        var.set(self.edge_visibility[triple_key])
                    else:
                        var.set(True)
                        self.edge_visibility[triple_key] = True
                    ...
                    delete_button.grid(row=row, column=0, sticky="w")
                    ...
                    checkbutton = ttk.Checkbutton(
                        ...
                        variable=var,
                        command=lambda tk=triple_key, v=var: self.update_edge_visibility(tk, v),
                    )
    ...
    def update_edge_visibility(self, triple_key, var):
        self.edge_visibility[triple_key] = var.get()
        self.display_image()

        for key in self.edge_visibility:
            self.edge_visibility[key] = False
            self.predicate_checkbuttons[key].set(False)
        self.display_image()
    ...
    def select_all_edges(self):
        ...
        for child in tab_widget.winfo_children():
            if isinstance(child, ttk.Checkbutton):
                ...
                self.edge_visibility[triple_key] = True
                self.predicate_checkbuttons[triple_key].set(True)
        self.display_image()
    ...
    def deselect_all_edges(self):
        ...
        for child in tab_widget.winfo_children():
            if isinstance(child, ttk.Checkbutton):
                ...
                self.edge_visibility[triple_key] = False
                self.predicate_checkbuttons[triple_key].set(False)
        self.display_image()
