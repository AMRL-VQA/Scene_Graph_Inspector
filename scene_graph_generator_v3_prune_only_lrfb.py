...
def prune_predicates(triples, num_criticals, id):
    ...
    while num_triples > 30:
        ...
        diff_count = {pred: predicate_count[pred] - num_criticals[pred] for pred in predicate_count if pred in num_criticals}
        if id == 70:
            print(f'diff: {diff_count}, triples: {num_triples}')

        # 필수 predicate들이 임계값에 다다르면 바로 return
        if all(diff_count[pred] == 0 for pred in diff_count):
            return triples
        
        # 가장 빈도수가 많은 predicate 그룹 선택, located in 과 holding은 제외
        for pred in diff_count:
            if diff_count[pred] > max_count:
                max_count = diff_count[pred]
                freq_pred = pred
...
def process_scene_graph(scene_graph_file_name, ambiguity_file_name):
    ...
    for image_data in tqdm(data):
        ...
        triples = prune_predicates(triples, num_criticals, id)
